...
export async function AvgHourBali({ from, to, interval = "hour" }: RangeProps): Promise<AvgWeatherData[]> {
    if (!from || !to) {
    ...
    try {
        ...
        console.log(`AvgHourBali - ${interval} query returned:`, result.length);
        return result || [];
    } catch (error) {
        console.error("Error in AvgHourBali:", error);
        return [];
    }
}
