...
export default async function PangandaranPage({
  ...
  try {
    const [data, avg] = await Promise.all([
      AvgGeneralHour({ from, to, interval, table: LOCATIONS.pangandaran.table }),
      ExportGeneric({ from, to, interval, table: LOCATIONS.pangandaran.table }),
    ]);
  ...
  return (
    <div className="w-full">
      <div className="max-w-7xl mx-auto">
        <PangandaranDashboard
          user={user}
          locationName={LOCATIONS.pangandaran.label}
          avgData={avgData}
